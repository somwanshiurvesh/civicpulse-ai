...
| `latitude` | `issues` | `DECIMAL(9,6)` | Latitude coordinate of the issue location (EPSG:4326). |
| `longitude` | `issues` | `DECIMAL(9,6)` | Longitude coordinate of the issue location (EPSG:4326). |
| `location` | `issues` | `GEOMETRY` | PostGIS spatial point (`SRID=4326`). Generated from latitude and longitude. Used for radius calculations. |
